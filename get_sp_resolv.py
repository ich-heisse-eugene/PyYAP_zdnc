...
def resol_in_order(wave, spec, n):
    ...
    lind,_ = find_peaks(signal/np.max(signal), distance=half_win, height=0.05)
    res = []
    wl = []
    dwl = []
    x = []
...
def make_report_resol(w, sp, input_file, view=False):
    ...
    with PdfPages(input_file.replace(".fits", ".report.pdf")) as pdf:
            ...
            for i in orders[page+(page*nplots): page+(page*nplots)+nplots]:
                ...
                if res_ord != -1:
                    ...
                    try:
                        ...
                        f_ax.set_xlim(-2*int(fwhm), 2*int(fwhm))
                    cur_col += 1
                    if cur_col > nx-1:
                        cur_col = 0
                        cur_row += 1
        ...
        try:
            ...
            ax.set_xlim(-2*int(fwhm), 2*int(fwhm))
        pdf.savefig()
        plt.close()
